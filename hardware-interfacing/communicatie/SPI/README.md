...
- [Een introductie](#een-introductie)
  - [Nieuwe variant: SPI met controller en peripheral](#nieuwe-variant-spi-met-controller-en-peripheral)
  - [Oude variant: SPI met één master en één slave](#oude-variant-spi-met-één-master-en-één-slave)
...
SPI (Serial Peripheral Interface) is een communicatiestandaard waarbij één chip, de controller (master), bepaalt welke van de peripherals (slaves) mag communiceren over de bus.

De SPI-aansluiting bestaat in haar meest simpele vorm uit twee datalijnen en een kloklijn.

De eerste datalijn heet MOSI (Master Out Slave In), hierover wordt de data van de master naar een slave gestuurd. De tweede datalijn is de MISO (Master In Slave Out), hierover wordt de data van een slave naar de master gestuurd. De kloklijn, SCLK (Serial CLocK), is er om aan te geven wanneer de volgende bit wordt verstuurd over de datalijn. Het signaal van de kloklijn wordt gegenereerd door de master.

> Om de associatie met slavernij te voorkomen is het voorstel gedaan om de pin referenties in relatie tot SPI aan te passen. [A Resolution to Redefine SPI Signal Names](<https://oshwa.org/resources/a-resolution-to-redefine-spi-signal-names/>). Deze terminologie is nog niet volledig overgenomen in dit document.
>
> **Overzicht**
>
>| New  | New Full Name                | Old  | Old Full Name       |
>|----  |----                          |----  | ----                |
...
### Nieuwe variant: SPI met controller en peripheral

![SPI met controller en peripheral](img/spi-comms.webp)

### Oude variant: SPI met één master en één slave

![SPI met één master en één slave.](img/SPI_master_slave.png)
...
![SPI met één master en drie slaves.](img/SPI_three_slaves.png)
...
![De aansluitingen van een 74HC595 output schuifregister](img/74HC595_intern.png)  ![](img/74HC595.png) ![](img/74HC595_img.png)
...
> Er kunnen ook andere reden zijn om een schuifregeister te gebruiken, bijvoorbeeld:
>
> - minder draden nodig van de micro-controller naar de plek waar de IO pinnen nodig zijn
> - de pinnen van een extender chip kunnen vaak meer stroom leveren dan die van een micro-controller
> - de pinnen van de micro-controller zijn op: goedkope micro-controllers hebben vaak veel minder IO pinnen, bijvoorbeeld een PIC10F200 heeft er maar 6 pinnen
...
Sluit de SD1306 OLED display als volgt aan op de Arduino Uno:

| SSD1306 pin | Arduino Uno pin|
| ---         | ---            |
| VCC         | 5V             |
| GND         | GND            |
| SCL (CLK)   | 13 (SCK)       |
...
- Waarom is er geen MISO verbinding?
- Wat zou de DC (DATA/COMMAND) verbinding voor zijn?
- Uitdaging: Wat is in de code een F() macro en waarom zouden we het gebruiken?

## De schakeling (met je team)

De volgende schakeling maak je **samen met je team**. Er is één  BME280 per team aanwezig.

De BME280 is een luchtdruk- en temperatuur en (relatieve)luchtvochtigheid sensor van Bosch.
De sensor kan gevoed worden met 3V3-5V.
De module heeft een logic level shifter voor de SDA en SCL lijn.
Je kunt dus ook aansluiten via [I<sup>2</sup>C](../I2C/README.md) op 3V3.
In dit voorbeeld demonsteren wij een aansluiting via (hardware) SPI.
De naamgeving kan verwarrend zijn.
Kijk naar de afbeelding voor de juiste benaming voor SPI bij deze module.
...
- [Arduino voorbeeld code voor BME280](files/bme280test/bme280test.ino)
...
- [Arduino voorbeeld code voor BMP280](files/bmp280test/bmp280test.ino)
